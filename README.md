...
Pri PDF so zlou textovou vrstvou možno zapnúť lokálny re-OCR fallback:

```bash
python3 scripts/extract_pdf_fulltext.py --ocr-mode bad-text --ocr-languages slk+ces --sync-articles
```

Režim najprv vyhodnotí text z `pdftotext` pomocou fontovej vrstvy (`pdffonts`) a
jazykových signálov chýbajúcej slovenskej/českej diakritiky. Ak je textová vrstva
podozrivá, článkové strany sa vyrenderujú cez `pdftoppm` a preženú cez Tesseract.
Tesseract sa spúšťa s `OMP_THREAD_LIMIT=1`, `OMP_NUM_THREADS=1` a predvolene s
`taskset -c 0`, aby nezaťažoval viac jadier. Marker/Surya je vhodný skôr na
selektívnu kontrolu layoutu a problémových strán, nie ako predvolený hromadný OCR
backend na CPU.

Po re-OCR je vhodné spustiť QA audit fulltextov:

```bash
python3 scripts/audit_fulltext_quality.py
```

Audit nemení zdrojový JSONL. Do `reports/fulltext_quality_audit.md` a
`reports/fulltext_quality_audit.json` zapíše prázdne alebo príliš krátke
fulltexty, nízku hustotu textu voči rozsahu strán, zvyškové znaky pokazenej
diakritiky, layoutové kandidáty na čistenie a duplicitné fulltextové skupiny.

Ak audit ukáže duplicitné `article_id`, najprv zrecykluj fulltextový JSONL:

```bash
python3 scripts/reconcile_fulltext_records.py --dry-run
python3 scripts/reconcile_fulltext_records.py
python3 scripts/audit_fulltext_quality.py
```

Reconcile vyberie jeden najlepší fulltextový riadok pre každý článok, zapíše
`reports/fulltext_reconcile_report.json` a pri in-place zápise vytvorí zálohu.
